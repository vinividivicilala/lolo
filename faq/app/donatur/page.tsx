...
const faqData = [
  ...
  },
  {
    ...
    answer: "Login ke akun Anda, klik tombol 'Buat Kegiatan', isi judul, deskripsi, target donasi, lokasi, nama lembaga, dan tanggal berakhir, lalu klik 'Buat'. Kegiatan donasi Anda akan langsung terlihat di feed."
  },
  {
...
// Arrow Icons untuk desain minimalis
...
interface DonationEvent {
  ...
  institutionName: string; // Nama PT/Lembaga
  location: string;
  endDate: Date;
  createdAt: Date;
  likes: string[];
...
// Instagram Verified Badge Component (tetap dipertahankan)
...
export default function DonationPage() {
  ...
  // State untuk Create Event - DITAMBAHKAN institutionName
  const [newEvent, setNewEvent] = useState({
    title: "",
    description: "",
    targetAmount: "",
    institutionName: "", // Nama PT/Lembaga
    location: "",
    endDate: "",
    category: "umum"
  });
  ...
  const exportToPDF = (event: DonationEvent) => {
    ...
    yPos += lineHeight;
    doc.text(`Lembaga: ${event.institutionName || event.organizerName}`, 20, yPos);
    yPos += lineHeight;
  ...
  // Create Event - DITAMBAHKAN institutionName
  const handleCreateEvent = async () => {
    ...
    try {
      const eventData = {
        ...
        institutionName: newEvent.institutionName.trim() || user.displayName || user.email?.split('@')[0] || 'Anonymous',
      ...
      setNewEvent({ title: "", description: "", targetAmount: "", institutionName: "", location: "", endDate: "", category: "umum" });
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const eventsData = snapshot.docs.map(doc => {
        ...
        return {
          ...
          institutionName: data.institutionName || data.organizerName || "",
  ...
  return (
    <>
                          ...
                          <div>
                            <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '6px' }}>
                              <span style={{ fontSize: '18px', fontWeight: '500', color: '#fff', fontFamily: 'Helvetica, Arial, sans-serif' }}>
                                {event.institutionName || event.organizerName}
                              </span>
                              <InstagramVerifiedBadge size={18} />
                            </div>
      ...
      {/* Create Event Modal - DITAMBAHKAN input institutionName (Nama PT/Lembaga) */}
      {showCreateModal && (
        <div style={{
          position: 'fixed',
          top: 0,
          ...
          <div style={{
            ...
            {/* Input Nama PT/Lembaga - DITAMBAHKAN */}
            <input
              type="text"
              value={newEvent.institutionName}
              onChange={(e) => setNewEvent({ ...newEvent, institutionName: e.target.value })}
              placeholder="Nama PT / Lembaga"
              style={{
                width: '100%',
                padding: '16px 0',
                border: 'none',
            ...
            >
              {isSubmitting ? 'Membuat...' : 'Buat Kegiatan'} <NorthEastArrow size={18} color="#000" />
            </button>
          </div>
        </div>
      )}
      ...
      {showStoryModal && selectedEventForStory && (
        <div style={{
          ...
          <div style={{
            ...
            >
              {isSubmitting ? 'Membagikan...' : 'Bagikan Cerita'} <NorthEastArrow size={18} color="#000" />
            </button>
          </div>
        </div>
      )}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: '16px', color: '#666', marginBottom: '32px', fontFamily: 'Helvetica, Arial, sans-serif' }}>
              {selectedEvent.title} — {selectedEvent.institutionName || selectedEvent.organizerName}
            </p>
            ...
            >
              {isSubmitting ? 'Memproses...' : 'Kirim Donasi'} <NorthEastArrow size={18} color="#000" />
            </button>
          </div>
        </div>
      )}
