...
export default function PusatBantuanPage() {
  ...
  const helpTitleRef = useRef<HTMLDivElement>(null);
  const contentRef = useRef<HTMLDivElement>(null);
    ...
    try {
      ...
      if (!querySnap.empty) {
        ...
        if (data) {
          ...
          setAuthorName(data.authorName || "Admin");
  ...
  useEffect(() => {
    ...
    const helpTitle = helpTitleRef.current;
    if (helpTitle) {
      gsap.fromTo(helpTitle,
        ...
        <div style={{
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              {sidebarOrder.map((category, index) => {
                const number = String(index + 1).padStart(2, '0');
