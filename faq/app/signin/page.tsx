...
export default function SignInPage({ onClose, onSwitchToSignUp, onSwitchToForgotPassword }: any) {
  ...
  const saveUserToLocalStorage = (userData: any, provider: string = 'email') => {
    try {
      const savedUsers = localStorage.getItem('savedLoginUsers');
      ...
      setLoginHistory(users.map((user: any) => ({
        id: user.uid,
        email: user.email,
        displayName: user.displayName,
        photoURL: user.photoURL,
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      saveUserToLocalStorage(user, 'google');
      
      // Firebase akan redirect otomatis via onAuthStateChanged
    } catch (error: any) {
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      saveUserToLocalStorage(user, 'github');
  ...
  const handleQuickLogin = async (savedUser: any) => {
    setLoading(true);
    setShowAutoLoginModal(false);
    
    try {
      // Untuk email login, kita perlu password
      if (savedUser.provider === 'email') {
        // Tampilkan form email/password dengan email yang sudah terisi
      ...
      // Untuk Google/GitHub, gunakan popup login
