...
// Menu items for drawer - TANPA STORIES
const menuItems = [
...
const LiveChatAgent = ({ user, isAdmin, db, auth }: { user: any; isAdmin: boolean; db: any; auth: any }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const senderName = isAdmin ? "Farid Ardiansyah" : (user.displayName || user.email || "User");
      ...
      await updateDoc(ticketRef, {
        ...
        agentName: isAdmin ? "Farid Ardiansyah" : selectedTicket.agentName,
      });
      setMessageText("");
      if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
    } catch (error) {
